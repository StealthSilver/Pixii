import type { ScrapedProduct } from "@/lib/marketEstimator/bestSellersScraper";

export type EstimatedProduct = ScrapedProduct & {
...
export function estimateRevenue(
  ...
  return products.map((p) => {
    const estimatedMonthlySales = estimateMonthlySales(p.rank, category);
    const estimatedMonthlyRevenue = estimatedMonthlySales * p.price;
    return {
      ...p,
