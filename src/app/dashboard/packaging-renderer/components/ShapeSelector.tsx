...
function ShapeIcon({ shape }: { shape: PackageShape }) {
  ...
  switch (shape) {
    case "box_square":
      return (
        <svg viewBox="0 0 48 48" className="size-10 text-neutral-700" aria-hidden>
...
export function ShapeSelector({ value, onChange }: ShapeSelectorProps) {
  return (
    <div>
      <p className="text-sm font-medium text-neutral-700">Package Shape</p>
        ...
        {PACKAGE_SHAPES.map((shape) => {
          ...
          return (
            <button
              ...
              className={
                "flex flex-col items-center rounded-xl border bg-white p-3 text-center shadow-sm transition-colors " +
                (active
            ...
            >
              ...
              <span className="mt-2 text-[11px] font-semibold leading-tight text-neutral-800">
                {LABELS[shape]}
              </span>
            </button>
          );
