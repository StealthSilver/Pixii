...
export function ScriptSections({ job }: ScriptSectionsProps) {
 ...
 type="button"
 onClick={() => setOpen((o) => !o)}
 className="flex w-full items-center justify-between gap-2 text-left font-heading text-sm font-semibold text-foreground"
 >
 <span className="flex items-center gap-2">
 <FaScroll className="size-4 shrink-0 text-muted-foreground" aria-hidden />
 ...
 <FaChevronDown className="size-4" aria-hidden />
 )}
 </span>
 </button>
 {open ? (
